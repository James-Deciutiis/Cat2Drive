...
const {client_id, client_secret, scope, redirect_uris, response_type} = require("./credentials.json");
const all_sessions = []
...
function redirect_to_drive(state, res){
	...
	let uri = querystring.stringify({scope, response_type, client_id, redirect_uri, state})
	res.writeHead(302, {Location: `${auth_endpoint}?${uri}`})
		.end();
}
...
function get_cat_picture_data(cat_breed, access_token, res){
	...
	https.request(cat_endpoint, {method:"GET"}, process_stream)
		.end()
	function process_stream(cat_stream){
		let cat_data = ""
		cat_stream.on("data", chunk => cat_data += chunk)
		cat_stream.on("end", () => download_cat_image(cat_data, downloaded_cat, access_token))
	}
}
...
function upload_new_image(filename, file_type, img_path, access_token){
	...
	const file = fs.readFile(img_path, function send_request(err, data){
		...
		const options = {
			...
			body: JSON.stringify({ data, mimeType: "application/vnd.google-apps.file" , name:"cat"}),
